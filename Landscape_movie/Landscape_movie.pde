...
void setup() {
  ...
  for (int pp=0; pp<ltot; pp++) {
    ...
    if (pieces.length == 5) { 
      z1[pp] = float(pieces[3]); // value to be represented from data
      z2[pp] = float(pieces[4]); // value to be represented from data
      ...
      sig[pp] = 6;//constrain(round(z1[pp]*0.5), 2, 30); // standard dev of each peak scales with value.

      if (abs(z1[pp] - z2[pp]) >= 5) {
        z_idxs.append(pp);
      }
    }
  }
...
void draw() {
  ...
  popMatrix();
  saveFrame("ipSC__neuron_keggdelta>5_####.png");
  if(counter >= 360) {
    noLoop();
    println("Finished");
  }
